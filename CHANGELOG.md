...
- Reconnecting keeps the deck you are on: the bridge re-reads the folder assignment before it replays the handshake, and checks which deck the new session is bound to. If a `bind_workspace` override or an elevated `switch_bound_deck` means the reconnected session would act on a different deck, the pending call is reported instead of retried there — re-bind and send it again.

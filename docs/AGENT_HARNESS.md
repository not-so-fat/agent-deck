...
**Order of operations:** `.agent-deck/use.json` makes Agent Deck a hard gate for that workspace. On the **first turn**, the harness requires `get_session_binding` and `get_bound_deck` before repo inspection or task action, shows `display_summary`, then loads every playbook whose trigger matches the task. If bootstrap fails, the agent stops instead of improvising without the deck. Terminal **status line** shows the bound deck after the MCP session registers its live display.
...
0. **Fail-closed bootstrap** — when `.agent-deck/use.json` exists, require `get_session_binding` → `get_bound_deck` before any task work; load matching playbooks and stop on connection or grant failure.
...
**Project scope** adds the explicit folder assignment via `agent-deck use <deck>`; match playbook `triggers` on `get_bound_deck` before improvising.

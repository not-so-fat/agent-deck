...
`RESOURCE_OUT_OF_SCOPE` carries optional `reason`: `deck_not_permitted` (mint/metadata scope escape), `deck_not_found` (id still in enrollment scope but deck deleted), or `tool_not_in_snapshot` (call).

...
- **`agent-deck-setup`** — triggers: "set up agent deck", "agent deck not connected". Body: diagnose daemon health, Codex plugin transport, folder assignment, and Codex instruction discovery separately. If the daemon is down, guide `npm i -g agent-deck && agent-deck start --daemon`. Codex transport comes from the enabled plugin's bundled `.mcp.json` running `agent-deck mcp-launch`. `agent-deck setup --client codex` marker-merges bootstrap guidance into global `~/.codex/AGENTS.md` (or project `./AGENTS.md`) without replacing other instructions; it does not install the plugin. A missing or legacy folder assignment is repaired with `agent-deck use <deck>`, followed by an MCP reload/retry. Read-only checks only; the *user* runs install commands.
